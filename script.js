...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!form) return; // إذا لم تكن الصفحة الحالية form.html
  ...
  form.addEventListener("submit", (e) => {
    ...
    const phone = document.getElementById("coordinatorPhone").value.trim();

    if (!/^\d{10}$/.test(id)) {
      msg.textContent = "❌ السجل المدني يجب أن يتكون من 10 أرقام.";
      return;
    }

    if (!/^05\d{8}$/.test(phone)) {
      msg.textContent = "❌ رقم الجوال يجب أن يبدأ بـ 05 ويتكون من 10 أرقام.";
      return;
    }
